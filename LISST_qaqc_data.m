function [cfg, data_proc,meta_proc] = LISST_qaqc_data(cfg,data_proc,meta_proc)
%function LISST_qaqc_downcasts runs quality checks on data
%
%  Syntax:
%    [cfg, data_proc] = LISST_qaqc_downcasts(cfg,data_proc)
%
%  Description:
...
if ~isfield(cfg.proc_options,'userprompted_qc')
  %**** SET TO 1 FOR QC TESTING/CONFIG! Automatically removes flagged points 
  cfg.proc_options.userprompted_qc = 0;
  fprintf('** Select QC Process **\n')
  fprintf('   <0> Auto QC - no user prompting \n')
  fprintf('   <1> User prompted QC - step through casts \n')
  cfg.proc_options.userprompted_qc = input('   Enter choice: ');
end
userprompted_qc = cfg.proc_options.userprompted_qc; % 
...
if ~isfield(cfg,'zscat_choice')
  if all(strcmp(cfg.zscfile,cfg.zscfile{1})) % single file was used
    [~,cfg.zscat_choice,~] = fileparts(cfg.zscfile{1});
  else
    cfg.zscat_choice = ''; % multiple background files used
  end
end
cfg.path.file_qc = strrep(cfg.path.file_proc,'.mat','_qc.mat'); % processed data will be saved here
%cfg.path.file_qc = fullfile(cfg.path.project,[cfg.project '_processed_qc.mat']); % processed data will be saved here

%% Set flag
flag = cfg.proc_options.quality_flag;

%% 2 | Initialize flag array and set all to not evaluated
data_proc.quality_flag        = flag.not_evaluated*ones(size(data_proc.data,1),1);
data_proc.quality_flag_test = repmat({''},size(data_proc.quality_flag));

meta_proc.quality_flag.desc      = 'data quality flag';
meta_proc.quality_flag_test.desc = 'reason for data quality flag, if anything besides not_evalulated or good';
% Auto generate flag metadata -- these are variable attributes, NOT global
flags = fieldnames(cfg.proc_options.quality_flag);
...
%% 3 | LOOP THROUGH TESTS DEFINED IN LISST_processing_config.m


%% Test 1 | Laser Reference values
% Laser Reference should never be 0 (or very close to 0, e.g. 0.02mW). If
% this is the case the laser is most likely dead. In this case only your
% pressure data and temperature data will be any good. The laser must be
% replaced. Contact Sequoia or your local distributor for instructions on
% what to do.
bad_laser_ref = data_proc.laserPower <= 0.02;
data_proc.quality_flag(bad_laser_ref)      = flag.bad;
data_proc.quality_flag_test(bad_laser_ref) = 'laserPower <= 0.02';
%% Test 2 | Transmission values
...
bad_trans = data_proc.tau < 0.10 | data_proc.tau > 0.995;
data_proc.quality_flag(bad_trans) = flag.bad;
...
% % que_trans1 = data_proc.tau > 1;
% % data_proc.quality_flag(que_trans1) = flag.questionable;
...
% below 0.30 (30%).
que_trans2 = data_proc.tau >= 0.10 & data_proc.tau <= 0.30;
data_proc.quality_flag(que_trans2) = flag.questionable;
% Be wary of data collected at transmission values between 0.98 and 0.995
% –low signal-to-noise ratio.
que_trans3 = data_proc.tau >= 0.98 & data_proc.tau <= 0.995;
data_proc.quality_flag(que_trans3) = flag.questionable;

%% Test 3 | Simple number distribution test
% simple QC from processLISST.m
isuspect = any(data_proc.VSD < 0,2);
